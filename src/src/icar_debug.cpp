...
#include "config_reader.cpp"                    //配置文件读取类
...
int main(int argc, char const *argv[])
{
  ...
  ConfigReader     configReader;                  //参数读取
  ...
  configReader.loadParams(); // 读取配置文件
  trackRecognition.rowCutUp = configReader.params.rowCutUp;
  trackRecognition.rowCutBottom = configReader.params.rowCutBottom;
  garageRecognition.disGarageEntry = configReader.params.disGarageEntry;

  if (configReader.params.GarageEnable) // 出入库使能
  ...
  while (1)
  {
    ...
    // [04] 出库和入库识别与路径规划
    if (configReader.params.GarageEnable) // 赛道元素是否使能
    {
          ...
          roadType == RoadType::BaseHandle)
      {
        ...
        if (circlesThis >= configReader.params.circles &&
    ...
    //[05] 农田区域检测
    if (configReader.params.FarmlandEnable) // 赛道元素是否使能
    {
    ...
    //[06] 维修厂检测
    if (configReader.params.DepotEnable) // 赛道元素是否使能
    {
    ...
    //[07] 粮仓检测
    if (configReader.params.GranaryEnable) // 赛道元素是否使能
    {
    ...
    // [08] 坡道（桥）检测与路径规划
    if (configReader.params.BridgeEnable) // 赛道元素是否使能
    {
    ...
    // [09] 慢行区检测与路径规划
    if (configReader.params.SlowzoneEnable) // 赛道元素是否使能
    {
    ...
    // [10] 泛行区检测与识别
    if (configReader.params.FreezoneEnable) // 赛道元素是否使能
    {
    ...
    // [11] 环岛识别与处理
    if (configReader.params.RingEnable) // 赛道元素是否使能
    {
    ...
    // [12] 十字道路处理
    if (configReader.params.CrossEnable) // 赛道元素是否使能
    {
...
//
    ...
    if (configReader.params.debug)
    {
      ...
      //        "v: " + formatDoble2String(configReader.motorSpeed, 2),
      //        Point(COLSIMAGE - 60, 80), FONT_HERSHEY_PLAIN, 1,
      //        Scalar(0, 0, 255), 1); // 车速

      string str = to_string(circlesThis) + "/" +
                   to_string(configReader.params.circles);
